...
function result(imageNum = 7) {
  ...
  const res = (char, num) =>
  {
	...
	if((num-1) % 3 === 0)
	{
		otpt = `<div class="result col-4 px-0 pb-1 d-flex"><div class="flex-fill row mx-0 result-mcontainer p-1 align-items-center border rounded-lg border-dark"><div class="d-flex left col-3 justify-content-end px-0">${num}位</div><div class="d-flex right col-9 pl-1 pr-0 justify-content-center"><div class="row mx-0"><div class="pl-1 pr-0 d-flex col-12 align-items-center text-break text-center" title="${charTooltip}">${charName}</div></div></div></div></div>`;
	}
	else
	{
